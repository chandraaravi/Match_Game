...
import ThumbnailItems from '../ThumbItems'
...
class MatchGame extends Component {
  ...
  changeImage = id => {
    const {matchedImg} = this.state

    if (id === matchedImg.id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        matchedImg: imagesList[randomIndex()],
      }))
    } else {
  ...
  newGame = () => {
    this.setState({timer: 60, displayResult: false})
    this.startTimer()
  }

  render() {
    const {score, timer, matchedImg, activeTabId, displayResult} = this.state
    const {imageUrl} = matchedImg
    ...
    const renderContent = () =>
      displayResult || timer === 0 ? (
        <div className="result-card">
          ...
          <p className="result-number">{score}</p>
          <button type="button" className="reset-btn" onClick={this.newGame}>
            <img
    ...
    return (
      <div className="bg-container">
        ...
        <div className="responsive-container">{renderContent()}</div>
      </div>
    )
  }
}
